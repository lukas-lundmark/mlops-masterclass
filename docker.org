...
* Docker
We will use Docker in every step of this Master Class. We will build Containers to run both locally for debugging as well as running in our CI/CD pipelines. Similarly, Azure ML will use docker to run its experiments locally in containerized environments.
